...
$empid = mysqli_real_escape_string($object->connect, $_POST["edit_empids"]);
...
if (!empty($_FILES['fileUploads']['name'])) {
    ...
    $old_filename = $_FILES['fileUploads']['name'];
    //$new_filename = $_FILES['fileUpload']['name'];
    ///----
    list($txt, $ext) = explode(".", $old_filename);
    $new_file_name = $empid . "." . $ext;
    //ตั้งชื่อใหม่
    copy($_FILES['fileUploads']['tmp_name'], "image/emp/" . $new_file_name);
