...
<form action="./pages/new_report_handover_stock.php" method="POST" target="_blank">
    <div class="row">
        <div class="col-1"></div>
        <div class="col-md-2">
            ...
            <div class="dropdown">
                <div class="form-group">
                    <select id="type" class="form-control" name="type">
                        ...
                        <option value="handover">ชื่อรายการส่งมอบ</option>
        ...
        <div class="col-md-1.5">
            <div class="dropdown">
                <div class="form-group">
                    <select name="handover" class="form-control" id="handover">
                        <option value="0">----เลือกรายการส่งมอบ----</option>
                        <?php
                        $sql_type1 = "SELECT * FROM tb_order  GROUP BY order_name  ORDER BY order_id ASC";
                        $re_type1 = mysqli_query($conn, $sql_type1);
                        while ($re_fac1 = mysqli_fetch_array($re_type1)) {
                        ?>
...
<div id="handover">
    <table id="g_handover" class="highchart" data-graph-type="column" style="display:none;" data-graph-datalabels-enabled="1" data-graph-datalabels-formatter="foo.myAwesomeCallback">
        <thead>
            <tr>
                <th>ประจำเดือน</th>
                <th>จำนวน</th>
...
<script>
    ...
    $('#btn_grap').click(function() {
        ...
        var handovers = $("#handover option:selected").text();
        ...
        if (select == 0) {
            ...
            $.ajax({
                url: "./pages/get_handover_stock.php",
                type: "POST",
                data: {
                    select: select,
                    st_date: st_date,
                ...
                success: function(data) {
                    ...
                    } else if (select == 'handover') {
                        $('#h').text('รายงานข้อมูลการส่งมอบ')
                        $('#label2').text('รายการส่งมอบ' + handovers)
                    } else {
                        $('#h').text('รายงานข้อมูลการส่งมอบ')
                    ...
                    for (var i = 0; i < len; i++) {
                        handover.Name.push(data[i].plant_name + " " + "เกรด" + data[i].grade_name);
                        handover.Amount.push(data[i].SUM1);


                    }
                    var max_count = Math.max.apply(Math, handover.Amount);
                    //get canvas
                    var ctx = $("#g_show");
                    //alert(max_count)
                    var data = {
                        labels: handover.Name,
                        datasets: [{
                            label: "จำนวนพันธุ์ไม้ (ต้น)",
                            data: handover.Amount,
                    ...
                    var options = {
                        ...
                        scales: {
                            ...
                            yAxes: [{
                                scaleLabel: {
                                    display: true,
                                    labelString: "จำนวนพันธุ์ไม้ที่ส่งมอบทั้งหมด (ต้น)",
    ...
    function load_member(select, st_date, en_date, month, month_year, year, customer, handover) {
        var firstname = $("#customer option:selected").text();
        var months = $("#month option:selected").text();
        var handovers = $("#handover option:selected").text();
        var st_date1 = new Date(st_date);
        ...
        var en_date2 = en_date1.toLocaleDateString();
        $.ajax({
            url: "./pages/report_handover_stock.php",
            method: "POST",
            data: {
                select: select,
                customer: customer,
    ...
    $('#btn_search').click(function() {
        ...
        if (select == 0) {
            ...
            $('#btn_grap').show()
            $('#g_show').hide()
            load_member(select, st_date, en_date, month, month_year, year, customer, handover)
