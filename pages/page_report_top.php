...
<form action="./pages/new_report_top.php" method="POST" target="_blank">
    <div class="row">
        <div class="col-1"></div>
        <div class="col-md-2">
...
<div id="top">
    <table id="g_top" class="highchart" data-graph-type="column" style="display:none;" data-graph-datalabels-enabled="1" data-graph-datalabels-formatter="foo.myAwesomeCallback">
        <thead>
            <tr>
                <th>ประจำเดือน</th>
                <th>จำนวน</th>
...
<script>
        ...
        if (select == 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (select == 'type') {
                        $('#h').text('รายงานข้อมูลพันธุ์ไม้ขายดี')
                    ...
                    for (var i = 0; i < len; i++) {
                        plant.Name.push(data[i].plant_name);
                        plant.SUM.push(data[i].SUM);


                    }
                    var max_count = Math.max.apply(Math, plant.SUM);
                    //get canvas
                    var ctx = $("#g_show");
                    //alert(max_count)
                    var data = {
    ...
    function load_member(select, st_date, en_date, month, month_year, year, type_plant) {
        var txt_type_plant = $("#type_plant option:selected").text();
        var months = $("#month option:selected").text();
        var st_date1 = new Date (st_date);
        var st_date2 = st_date1.toLocaleDateString();
    ...
    $('#btn_search').click(function() {
        ...
        if (select == 0) {
            ...
            $('#btn_grap').show()
            $('#g_show').hide()
            load_member(select, st_date, en_date, month, month_year, year, type_plant)
