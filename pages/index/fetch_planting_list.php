...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $subdata[] = $sum_dead ;
        $subdata[] = $total_price ;
        $subdata[] = $row['planting_week_date'];
        ...
        <button type="button" class="btn btn-info btn-sm" id="btn_planting_week_detail"
            data-toggle="tooltip"  title="แสดงรายละเอียดการปลูก" data-id ="' . $row['planting_week_id'] . '"
            data-status="' . $row['planting_week_status'] . '"  data-name="' . $row['plant_name'] . '" data-count="' . $row['planting_week_count'] . '">
