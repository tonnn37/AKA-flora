...
WHERE (tb_order_detail.order_detail_status ='ปกติ' OR tb_order_detail.order_detail_status ='รอส่งมอบ')
GROUP BY order_id 
ORDER BY tb_order.order_id DESC";
    ...
    LEFT JOIN tb_plant ON tb_plant.plant_id = tb_order_detail.ref_plant_id
    LEFT JOIN tb_customer ON tb_customer.customer_id = tb_order.order_customer
    WHERE tb_order_detail.order_detail_status ='เสร็จสิ้น' 
    GROUP BY order_id 
    ORDER BY tb_order.order_id DESC";
}else{
    $sql = "SELECT tb_order.order_id as order_id, tb_order.order_name as order_name,
    ...
    LEFT JOIN tb_plant ON tb_plant.plant_id = tb_order_detail.ref_plant_id
    LEFT JOIN tb_customer ON tb_customer.customer_id = tb_order.order_customer
    WHERE tb_order_detail.order_detail_status ='ระงับ' 
    GROUP BY order_id 
    ORDER BY tb_order.order_id DESC";
}
